...
def find_window_centroids(image, window_width, window_height, margin):
    ...
    for level in range(1,(int)(warped.shape[0]/window_height)):
        image_layer = np.sum(warped[int(warped.shape[0]-(level+1)*window_height):int(warped.shape[0]-level*window_height),:], axis=0) 	# convolve the window into the vertical slice of the image
...
if 1:
    ...
    warped, Minv = warp(filtrdImg) #Bird Eye view

    histogram = np.sum(warped[warped.shape[0]/2:,:] > 0, axis=0)
    xx = np.zeros((warped.shape[0], warped.shape[1], 3))
    xx[:,:,0] = warped > 0
    #plt.imshow(xx);    plt.plot(histogram);    plt.show()
